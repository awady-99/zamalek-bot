...
Architecture
------------
1. Fetcher      - resilient HTTP layer (curl_cffi, browser-impersonating TLS
                   fingerprint) with an optional Playwright fallback for pages
                   that only render ticket status via JavaScript.
2. Parser       - BeautifulSoup-based extraction, driven entirely by CSS
                   selectors defined in the .env file so you can adapt it to
                   Tazkarti's actual markup without touching code.
3. StateStore   - a small JSON file on disk that remembers the last known
                   status of every match so we only alert on a *change*
                   (unseen -> available, sold-out -> available), not on every
                   poll.
4. Notifier     - thin wrapper around the Telegram Bot API's sendMessage
                   endpoint, with retries.
5. Orchestrator - the asyncio loop that ties everything together, with
                   jittered polling, exponential backoff on errors, and a
                   self-throttling circuit breaker to avoid IP bans.

IMPORTANT
---------
Tazkarti's exact HTML structure was not available to inspect at the time this
script was written (the site returns a JS-rendered shell to automated
fetchers, which is exactly why the Playwright fallback + configurable
selectors exist). Before running this for real you MUST:

  1. Open the Tazkarti matches/events page in a real browser.
  2. Use "Inspect Element" on a match card, its title, its date, its status
     badge, and its "Book Now" button/link.
